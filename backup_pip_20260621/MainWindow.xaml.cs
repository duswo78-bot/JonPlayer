    ...
    public partial class MainWindow : Window
{
        ...
        {
            if ((msg == 0x0046 || msg == 0x0047) && _overlayWindow != null) // WM_WINDOWPOSCHANGING / WM_WINDOWPOSCHANGED
            {
                var helper = new WindowInteropHelper(_overlayWindow);
                if (helper.Handle != IntPtr.Zero)
                {
                    WINDOWPOS pos = Marshal.PtrToStructure<WINDOWPOS>(lParam);
                    _ = SetWindowPos(helper.Handle, IntPtr.Zero, pos.x, pos.y, pos.cx, pos.cy, pos.flags | 0x0014); // Combine with SWP_NOACTIVATE | SWP_NOZORDER

                    var source = PresentationSource.FromVisual(this);
                    if (source != null && source.CompositionTarget != null)
                    {
                        var transform = source.CompositionTarget.TransformFromDevice;
                        var point = transform.Transform(new System.Windows.Point(pos.x, pos.y));
                        var size = transform.Transform(new System.Windows.Point(pos.cx, pos.cy));
                        
                        if ((pos.flags & 0x0002) == 0) // SWP_NOMOVE
                        {
                            _overlayWindow.Left = point.X;
                            _overlayWindow.Top = point.Y;
                        }
                        if ((pos.flags & 0x0001) == 0) // SWP_NOSIZE
                        {
                            _overlayWindow.Width = size.X;
                            _overlayWindow.Height = size.Y;
                        }
                    }
                }
            }
